...
function renderDeckArea() {
  ...
  if (state.drawnCards.length === 0) {
    ...
    for (const card of state.drawnCards) {
      ...
      element.className = `draw-card ${card.tone}`;
      ...
      element.innerHTML = `
        <button type="button" class="draw-card-preview-button expand-drawn-card-button" data-card-id="${escapeHtml(card.id)}" aria-label="Expand ${escapeHtml(card.title)}">
...
function expandDrawnCard(cardId) {
  ...
  ui.characterCardOverlayImage.dataset.fallbackSrc = card.fallbackImage || getFallbackCardImagePath(card.image);
